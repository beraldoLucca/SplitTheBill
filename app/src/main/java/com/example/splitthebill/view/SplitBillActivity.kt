    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        personList?.let { persons ->
            for (person in persons){
                sum += person.paidValue
            }
            val payPerPerson = (sum / persons.size)

            for (person in persons){
                val payOrReceive = payPerPerson - person.paidValue
                person.valueToPay = if ((payOrReceive) > 0) payOrReceive else 0.0
                person.valueToReceive = if ((payOrReceive) < 0) -payOrReceive else 0.0
            }
